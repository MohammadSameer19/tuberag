...
class ChromaDBVideoRAG:
    ...
    def __init__(self, perplexity_api_key: str, persist_dir: str = "./chroma_db"):
        ...
        # Use multilingual sentence-transformers embedding (supports 100+ languages!)
    ...
    def _get_query_embedding(self, text: str) -> List[float]:
        """Get embedding for query text using local sentence-transformers."""
    ...
    def _fetch_transcript(self, video_id: str) -> Optional[str]:
        ...
        try:
            ...
            if transcript_text and transcript_text.strip():
                logger.info(f"Successfully fetched transcript for video {video_id} ({len(transcript_text)} characters)")
                return transcript_text
            else:
                logger.error(f"No transcript text retrieved for video {video_id}")
                return None
        except Exception as e:
    ...
    def load_video(self, video_id: str) -> bool:
        ...
        try:
            ...
            # Create embeddings using local model (no rate limits!)
            embeddings = []
...
IMPORTANT: Respond in the SAME LANGUAGE as the question. If the question is in Korean, respond in Korean. If in Japanese, respond in Japanese. If in Spanish, respond in Spanish, etc.
